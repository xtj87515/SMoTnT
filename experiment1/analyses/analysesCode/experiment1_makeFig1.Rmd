...
PS_byTimeRep <- function(gName1, gName2, gName3)
{
    ...
    p1 <- protSynRepTime %>%
      ...
      scale_color_manual(name="", labels=c("mRNAconstant"="mRNAconstant", "dc_1_r_0"="Co-trans 100%"), values = c("#1F78B4", "#FF7F00")) +
      scale_y_continuous(expand = c(0, 0), name="Real-time mRNA abundance (single replicate)", sec.axis = sec_axis(~.*(coeff[[1]]*coeff[[2]]*coeff[[3]]), name = "Protein synthesized per minute"))  # remove the extra space on the axis
...
figure1e <- (allSimInputOutput %>%
  filter(paraCombo=="dc_1_r_0", ORF!="YER053C-A"))[, c("ORF", "RMPSR", "RMVPS")] %>% #remove the gene with the 450min HL
  ...
  theme(text = element_text(size = 13),
        ...
        plot.margin = margin(r = 0, unit = "pt"),
        panel.grid = element_blank()) +
    scale_color_manual(values = c("RMPSR"="black", "RMVPS"="black")) +  
    scale_linetype_manual(name = "", labels=c("RMPSR"="Rel. mean of prot.syn.rates",
                                              "RMVPS"="Rel. variance of prot.syn.rates"),
                          values = c("RMPSR"=1, "RMVPS"=3)) + 
    scale_x_log10()
#################################################################
figure1f <-(allSimInputOutput %>%
  filter(paraCombo=="dc_1_r_0", ORF!="YER053C-A", mRNADecRateNeymotin_sec!=0))[, c("ORF", "CVPS", "CVMC", "paraCombo")] %>% #remove the gene with the 450min HL
